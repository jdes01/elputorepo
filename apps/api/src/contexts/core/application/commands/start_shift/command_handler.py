...
class StartShiftCommand(Schema):
    shift_id: str


class StartShiftResult(Schema):
...
@dataclass
class StartShiftCommandHandler:
    shift_repository: ShiftRepository
    settings: Settings
    event_bus: EventBus

    def handle(self, command: StartShiftCommand) -> StartShiftResult:
        logger.info(
            "Handling StartShiftCommand",
        ...
        return StartShiftResult(shift=shift.to_primitives())
