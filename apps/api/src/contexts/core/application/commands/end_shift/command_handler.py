...
class EndShiftCommand(Schema):
    shift_id: str


class EndShiftResult(Schema):
...
@dataclass
class EndShiftCommandHandler:
    shift_repository: ShiftRepository
    settings: Settings
    event_bus: EventBus

    def handle(self, command: EndShiftCommand) -> EndShiftResult:
        logger.info(
            "Handling EndShiftCommand",
        ...
        return EndShiftResult(shift=shift.to_primitives())
