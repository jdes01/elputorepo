...
export class AcquireTicketCommandHandler {
  ...
  async handle(command: AcquireTicketCommand): Promise<{ ticketId: string }> {
    ...
    if (!event.hasAvailableTickets(1)) {
      this.logger.warn(
        `No available tickets for event ${eventId.value}. Available: ${event.availableTickets}`,
      );
    ...
    this.logger.log(
      `Ticket acquired: ${ticket.id.value} for event ${eventId.value} and user ${userId.value}`,
    );
